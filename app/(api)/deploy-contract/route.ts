import { NextResponse, NextRequest } from "next/server";
import { ethers } from "ethers";
import { ExternalProvider } from "@ethersproject/providers";
declare var window: any
const { Network, Alchemy, Wallet, ContractFactory } = require("alchemy-sdk");



export async function POST(req: NextRequest) {
    //const { contractName, contractABI, contractBytecode } = await req.json();
    const settings = {
        apiKey: "uVDZyUXEkTm7v0E9m83VIJq8yOidXWNM", // Replace with your Alchemy API Key.
        network: Network.ETH_SEPOLIA, // Replace with your network.
    };

    const alchemy = new Alchemy(settings);
    const contractNameTest = "SecureEfficientSwap";
    const contractABITest = [
        {
            "inputs": [
                {
                    "internalType": "uint256",
                    "name": "_gumballinitial",
                    "type": "uint256"
                }
            ],
            "stateMutability": "nonpayable",
            "type": "constructor"
        },
        {
            "inputs": [
                {
                    "internalType": "uint256",
                    "name": "_gumballadd",
                    "type": "uint256"
                }
            ],
            "name": "addFreshGumballs",
            "outputs": [],
            "stateMutability": "nonpayable",
            "type": "function"
        },
        {
            "inputs": [],
            "name": "getGumball",
            "outputs": [],
            "stateMutability": "nonpayable",
            "type": "function"
        },
        {
            "inputs": [],
            "name": "getNumberOfGumballs",
            "outputs": [
                {
                    "internalType": "uint256",
                    "name": "",
                    "type": "uint256"
                }
            ],
            "stateMutability": "view",
            "type": "function"
        }
    ];
    const contractBytecodeTest = "";

    const provider = new ethers.providers.AlchemyProvider("mainnet", "uVDZyUXEkTm7v0E9m83VIJq8yOidXWNM");
    // let privateKey = process.env.PRIVATE_KEY;
    // let wallet = new Wallet(privateKey, provider);

    // // Create an instance of a Contract Factory
    // let factory = new ContractFactory(contractABITest, contractBytecodeTest, wallet);
    // let contract = await factory.deploy(100)
    // console.log(contract.address);

    // console.log(contract.deployTransaction.hash);

    // await contract.deployed();
    const output = await provider.getBlockNumber();
    console.log(output);



    return new NextResponse(JSON.stringify({ message: "Contract deployed" }), { status: 200 });
}
