...
contract ContractDemo {
    ...
    event SwapInitialized(address initiator, uint256 amountIn, address tokenIn, address tokenOut);
    ...
    function swapTokens(uint256 amountIn) external {
        ...
        require(IERC20(token1).transferFrom(msg.sender, address(this), amountIn), "Token transfer failed");

        // Assuming a 1:1 swap ratio for simplicity
        uint256 amountOut = amountIn;

        // Transfer token2 from this contract to msg.sender
        require(IERC20(token2).balanceOf(address(this)) >= amountOut, "Insufficient token2 balance in contract");
        require(IERC20(token2).transfer(msg.sender, amountOut), "Token transfer failed");
